        ...
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="glass-panel rounded-[2rem] p-5 sm:p-6">
                <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <form method="GET" action="{{ route('messages.conversation', $user) }}" class="flex w-full flex-col gap-3 sm:flex-row lg:max-w-xl">
                        <div class="flex-1">
            ...
            <section class="glass-panel rounded-[2rem] p-4 sm:p-6">
                ...
                @forelse ($messages as $message)
                    <article class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-3xl {{ $message->sender_id === auth()->id() ? 'items-end' : 'items-start' }} flex w-full flex-col gap-2">
                            <div class="flex items-center gap-3 {{ $message->sender_id === auth()->id() ? 'flex-row-reverse' : '' }}">
                                <x-user-avatar :user="$message->sender" class="h-10 w-10 shrink-0 bg-[var(--brand)] text-sm font-semibold uppercase text-white shadow-[0_10px_24px_rgba(79,70,229,0.22)]" />
                            ...
                            <div class="{{ $message->sender_id === auth()->id() ? 'rounded-[1.75rem_1.75rem_0.5rem_1.75rem] bg-[var(--brand)] text-white' : 'rounded-[1.75rem_1.75rem_1.75rem_0.5rem] bg-white text-stone-800' }} w-full px-5 py-4 shadow-[0_18px_36px_rgba(15,23,42,0.08)]">
                                <p class="whitespace-pre-line text-base leading-8">{{ $message->content }}</p>
                            </div>
                            <div class="flex items-center gap-3 text-xs text-stone-500 {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                                <span>{{ $message->created_at->format('d/m/Y H:i') }}</span>
                                <form method="POST" action="{{ route('messages.destroy', $message) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-full border border-rose-200 bg-rose-50/90 px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-[0.16em] text-rose-700 transition hover:bg-rose-100">
                                        Supprimer
                                    </button>
                                </form>
                            </div>
                ...
                @empty
                    <div class="rounded-[2rem] border border-dashed border-[rgba(71,85,135,0.16)] bg-white/70 p-12 text-center">
                        ...
                        <p class="mt-3 text-sm text-stone-500">Envoyez le premier message pour lancer l'echange.</p>
                    </div>
                @endforelse
                </div>
            </section>
