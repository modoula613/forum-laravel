<x-app-layout>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="glass-panel rounded-[2rem] p-5 sm:p-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <h3 class="mt-2 text-2xl font-semibold text-stone-950">Conversations recentes</h3>
                        <p class="mt-2 text-sm leading-6 text-stone-500">
                            Retrouve rapidement la bonne personne, les messages non lus et les notifications importantes.
                        </p>
                    </div>
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,0.95fr)]">
                <section class="space-y-4">
                    @forelse ($conversations as $conversation)
                        <article class="glass-panel rounded-[2rem] p-5 transition hover:-translate-y-0.5 hover:shadow-[0_18px_40px_rgba(15,23,42,0.08)] sm:p-6">
                            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                                <div class="flex min-w-0 items-start gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-3">
                                            ...
                                            <span class="rounded-full {{ $conversation->unread_count > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700' }} px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em]">
                                                {{ $conversation->unread_count > 0 ? $conversation->unread_count.' non lu(s)' : 'A jour' }}
                                            </span>
                                        </div>
                                        <div class="mt-3 flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">
                                            <span>{{ $conversation->last_message->sender_id === auth()->id() ? 'Dernier message envoye' : 'Dernier message recu' }}</span>
                                            <span>{{ $conversation->last_message->created_at->format('d/m/Y H:i') }}</span>
                                        </div>
                                        <div class="mt-4 rounded-[1.5rem] bg-[rgba(248,250,252,0.9)] px-4 py-4">
                                            <p class="text-sm font-semibold uppercase tracking-[0.16em] text-stone-500">
                                                Apercu
                                            </p>
                                            <p class="mt-2 whitespace-pre-line text-base leading-7 text-stone-700">
                                                {{ \Illuminate\Support\Str::limit($conversation->last_message->content, 220) }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="flex shrink-0 flex-col items-start gap-3 lg:items-end">
                                    ...
                                    @endif
                                    <a href="{{ route('messages.conversation', $conversation->user) }}" class="inline-flex rounded-full bg-[var(--brand)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[var(--brand-deep)]">
